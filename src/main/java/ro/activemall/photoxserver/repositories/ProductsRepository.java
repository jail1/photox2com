...
public interface ProductsRepository extends JpaRepository<Product, Long> {
	...
	@Query("SELECT p FROM product p WHERE p.isListable = 1 AND p.isComplex = 1 ORDER BY p.ratio")
	public List<Product> getAlbumProducts();
	
	@Query("SELECT p FROM product p WHERE p.isListable = 1 AND p.isComplex = 0")
	public List<Product> getPictureProducts();
}
