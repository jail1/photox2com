...
public class UsersService extends ApplicationAbstractService {
	...
	public User createAccountOnlyWithEmail(String email) {
		...
		if (existingUser == null) {
			...
			UserActionEvent event = new UserActionEvent(user, UserActivityTypes.REGISTER.getIntValue(), new PasswordAndTokenPO(password, token));
	...
	public User updateUserProfile(User user, User authenticatedUser) {
		...
		if (user.getChangePasswordTo() != null) {
			...
			UserActionEvent event = new UserActionEvent(authenticatedUser, UserActivityTypes.CHANGED_PASSWORD.getIntValue(), user.getChangePasswordTo());
			publisher.publishEvent(event);
