...
public class RawMaterialsService extends ApplicationAbstractService {
	...
	public RawMaterial createOrUpdateRawMaterial(Long groupId, RawMaterial rawmaterial) {
		...
		if (rawmaterial.getId() == null || rawmaterial.getId().compareTo(0L) == 0){
			//add price zero if parent group is a selector
			if (parentGroup.getIsSelector()){
				repository.save(rawmaterial);
				needsSave = false;
				String sqlQuery = "SELECT product_id FROM photox_products_selectors WHERE rawmaterialgroup_id = :rawmaterialgroup_id";
				Query query = entityManager.createNativeQuery(sqlQuery);
				query.setParameter("rawmaterialgroup_id", parentGroup.getId());				
				List<BigInteger> productIds = (List<BigInteger>) query.getResultList();
				for (BigInteger productId : productIds){
					//TODO : see WTH is going on here!!!
					log.info("Product id "+productId+" needs zero price for this raw material");
					ProductPrice productPrice = pricesRepository.findPriceForProductAndMaterial(productId.longValue(), rawmaterial.getId());						
					if (productPrice == null){
						Product targetProduct = productsRepository.findOne(productId.longValue());
						productPrice  = new ProductPrice();
						productPrice.setProduct(targetProduct);
						productPrice.setMaterial(rawmaterial);
						productPrice.setSellingPrice(0d);
						pricesRepository.save(productPrice);
					}else{
						log.error("Price already declared");
					}
				}
			}	
		}
