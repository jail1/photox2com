...
public class ProductsService extends ApplicationAbstractService {
	...
	public Product cloneProduct(Long productId, String[] data) {
		...
		if (clonedProduct.getIsComplex()) {		
			for (Product child : clonedProduct.getChildren()){				
				child.setRatio(clonedProduct.getLostRatio());
				child.setExtraData(clonedProduct.getExtraData());
			}
	...
	public Product updateProductExtra(Long id, ExtraDataJSON newExtra){
		Product product = repository.findOne(id);
		product.setExtraData(newExtra.toJsonString());
		product.setRatio(Double.valueOf(newExtra.width * newExtra.height) / 1000000);
		return repository.save(product);
	}
	
	public List<ProductJSON> getAllListedProducts(){
		List<Product> listedProducts = repository.getAllListedProducts();
	...
	public List<ProductJSON> getAlbumProducts(){
		List<Product> listedProducts = repository.getAlbumProducts();
		List<ProductJSON> result = new ArrayList<ProductJSON>();
		for (Product product : listedProducts){			
			ProductJSON endProduct = new ProductJSON(product);
			result.add(endProduct);
	...
	public List<ProductJSON> getPictureProducts(){
		List<Product> listedProducts = repository.getPictureProducts();
		List<ProductJSON> result = new ArrayList<ProductJSON>();
		for (Product product : listedProducts){			
			ProductJSON endProduct = new ProductJSON(product);
			result.add(endProduct);
