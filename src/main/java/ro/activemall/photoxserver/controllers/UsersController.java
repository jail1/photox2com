...
import ro.activemall.photoxserver.services.UsersExtraService;
...
public class UsersController {
	...
	@Autowired
	UsersExtraService extraService;
	...
	public String authenticatedPrincipal(@CurrentlyLoggedUser User authenticated) {
		...
		try {
			...
			extraAsJson = mapper.writeValueAsString(extraService.getExtraDataByOwnerId(authenticated.getId()));
		} catch (Exception ex) {
			System.out.println(ex.getStackTrace());
		}
		String result = "var loggedinUser = " + userAsJson + ";";
		result += (extraAsJson == null ? "" : ";\nvar loggedinUserExtra =" + extraAsJson+";\n");
		result += "var serverSettings = {";
		List<PlatformSetting> settings = settingsRepository.findAll();
		int counter = settings.size();
