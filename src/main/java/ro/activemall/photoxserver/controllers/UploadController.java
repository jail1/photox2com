...
public class UploadController {
	...
	@Secured({ RolesAsStrings.ROLE_SUPER_ADMIN, RolesAsStrings.ROLE_PHOTOGRAPHER,
		RolesAsStrings.ROLE_PHOTOGRAPHERS_CLIENT })
	@RequestMapping(method = RequestMethod.POST, value = ApiUrls.TESTRESOURCEUPLOAD, produces = ApiUrls.JSON)
	@ResponseBody
	public HttpEntity<byte[]> testUpload(@RequestPart MultipartFile file, @CurrentlyLoggedUser User loggedInUser)
	{
		return service.testUpload(file, loggedInUser);
	}
	...
	public ResponseEntity<String> getResource(@PathVariable String fileName, @CurrentlyLoggedUser User loggedinUser) {
	...
	public HttpEntity<byte[]> getResource(@PathVariable String fileName, WebRequest request, @CurrentlyLoggedUser User loggedinUser) throws IOException {
