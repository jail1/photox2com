(function () {
    ...
    function adminAddOrEditComposedProductCtrl($scope, $rootScope, $cookies, $state, $stateParams, commons, localize, $mdDialog, ProductsService) {
        ...
        function postSimpleProductAdded(nodeData) {
            ...
            if (nodeData.prices.length > 0) {
                nodeData.prices.forEach(function(price){
                    //place a server call to store selling price inherited, so we won't have blank prices
                    ProductsService.storeSellingPrice($scope.product.id, price.materialId, price.sellingPrice);
                });
            }
        }
        ...
        $scope.basicProductEdited = function (basicProduct) {
            ProductsService.updateProductName(basicProduct.id, basicProduct.name).then(function (serverResponse) {
                commons.log("Saved basic product field");
                commons.log(serverResponse);
            });
        };

        $scope.showExtraDataDialog = function(ev, basicProduct){
            ev.stopImmediatePropagation();
            var dialogScope = $scope.$new();
            dialogScope.product = angular.copy(basicProduct);
            $mdDialog.show({
                controller: DialogController,
                templateUrl: 'measuresProductTemplateDialog.html',
                targetEvent: ev,
                scope: dialogScope
            }).then(function (savedProduct) {
                ProductsService.updateProductExtra(savedProduct.id , savedProduct.extraData).then(function (serverResponse) {
                    //because ratio changes, we need to re-make calculus
